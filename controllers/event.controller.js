...
// @route   GET /api/v1/exhibitions/:exhibitionId/events
// @access  Public
exports.getEvents = asyncHandler(async (req, res, next) => {
  ...
  if (req.params.exhibitionId) {
    // Get events for specific exhibition
    query = Event.find({ exhibition: req.params.exhibitionId });
  } else {
    // Get all events
    query = Event.find();
  }
...
exports.getEvent = asyncHandler(async (req, res, next) => {
  const event = await Event.findById(req.params.id)
    .populate('exhibition', 'name venue startDate endDate')
